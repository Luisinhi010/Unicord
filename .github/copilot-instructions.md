...
6. Build Release x64 only. Do not spend engineering effort on x86, ARM32, Windows Mobile, Xbox, or old Windows versions unless the legacy UWP packaging toolchain requires metadata for successful packaging.
7. Keep UWP compatibility quirks that are required by the build system (for example required manifest metadata) even when the corresponding legacy platform is not a target.
